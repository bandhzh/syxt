(function ($) {
  function SlickGridPager(dataView, grid, $container, options) {
    ...
    if(options.filter){
    	__filter.filters = options.filter;
    }
    var h_request = null;
    ...
    function ajaxGetData(fromPage){
    	var param = $.extend(true,__filter,{
            ...
            limit : opts.pagesize,
            page:{rowStart:fromPage * opts.pagesize,limit:opts.pagesize}
        });
    	if (h_request != null) {
    	...
    	h_request = setTimeout(function () {
    		...
    		var _param = {data:JSON.stringify(param),_time_stamp_:new Date().getTime()}
    		req = $.ajax({
    			type : "POST",
    	        url: opts.url,
    	        async:true,
